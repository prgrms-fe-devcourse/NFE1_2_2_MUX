...
```
git checkout -b feature/자신이 생성한 이슈 번호(ex. #1)/자신의 이름
```
...
```
git push -u origin feature/자신이 생성한 이슈 번호(ex. #1)/자신의 이름
```
...
```
git branch -d feature/자신이 생성한 이슈 번호(ex. #1)/자신의 이름  # 로컬에서 삭제
git push origin --delete feature/자신이 생성한 이슈 번호(ex. #1)/자신의 이름  # 원격에서 삭제
```
